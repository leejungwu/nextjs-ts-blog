...
const Interview: React.FunctionComponent = () => {
  ...
  const pagedData = paginate(interviewPosts, currentPage, pageSize);
  ...
  return (
      <>
        ...
        <Layout>
            <Wrapper>
              <h1 style={{ textAlign: 'center' }}>면접준비</h1>
              <br/>
                ...
                <table className="table">
                  ...
                  {pagedData.map((data) => (
                      <tr key={data.id}>
                        {data.title ? (
                          <td><a onClick={() => {
                            router.push(`/category/interview/post/${data.id}`)
                            dispatch({
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                <Pagination 
                pageSize={pageSize} 
                itemsCount={interviewPosts.length} 
